...
class AcaiTest {
    ...
    void testToString() throws TamanhoNaoExisteExcepition, AdicionalInvalidoException {
        ...
        assertEquals("Açaí{ Tamanho = P, adicionais = LEITECON}", produto.toString());

        Produto produto2= new Acai('G', 5);
        assertEquals("Açaí{ Tamanho = G, adicionais = KIWI}", produto2.toString());

        Produto produto3= new Acai('P', 6);
        assertEquals("Açaí{ Tamanho = P, adicionais = NENHUM}", produto3.toString());
    }
